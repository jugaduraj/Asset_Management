...
export default function EmployeesPage() {
  ...
  const handleEmployeeSubmit = async (employee: Omit<Employee, '_id' | 'createdAt'> | Employee) => {
    const isEditing = '_id' in employee;
    const url = '/api/employees';
    let body;
    let method;

    if (isEditing) {
        const { _id, ...employeeData } = employee as Employee;
        method = 'PUT';
        body = JSON.stringify({ id: _id, ...employeeData });
    } else {
        method = 'POST';
